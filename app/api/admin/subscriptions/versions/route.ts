 ...
 * Get version history for subscription configuration
 *
 * Query params:
 * - limit?: number (default: 20)
 *
 * Returns:
 * - versions: SubscriptionConfigVersion[]
 * - total: number
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const versions: SubscriptionConfigVersion[] = versionsSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    } as SubscriptionConfigVersion));
    ...
    console.error('[Admin Subscriptions Versions API] Error fetching versions:', error);
...
/**
 * POST /api/admin/subscriptions/versions
 ...
 *
 * Returns:
 * - config: SubscriptionTierConfig (rolled back config)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const targetVersion = targetVersionDoc.data() as SubscriptionConfigVersion;
    ...
    const currentConfig = currentConfigDoc.data() as SubscriptionTierConfig;
    const now = new Date().toISOString();
    const newVersion = currentConfig.version + 1;

    // Create rolled back config (copy tiers and settings from target, but new version/timestamp)
    const rolledBackConfig: SubscriptionTierConfig = {
    ...
    console.error('[Admin Subscriptions Versions API] Error rolling back:', error);
