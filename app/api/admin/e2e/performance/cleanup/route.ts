...
import { cleanupPerformanceData } from '@/lib/services/e2e/perfOperations';
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        await cleanupPerformanceData(db, send, config.days || config.metricsPerUserPerDay ? { days: config.days ?? 7, metricsPerUserPerDay: config.metricsPerUserPerDay ?? 30 } : undefined);
      } catch (error: any) {
        send({
          phase: -1,
          phaseName: 'Error',
          level: 'error',
          message: `Performance cleanup failed: ${error.message}`,
        });
      } finally {
        controller.close();
      }
