        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                ...
                notification.status === 'opened' ? 'bg-emerald-100 text-emerald-800' :
                notification.status === 'suppressed' ? 'bg-yellow-100 text-yellow-800' :
          ...
          <div className="grid grid-cols-2 gap-4">
            {notification.channel && (
              <div>
                ...
                <code className="text-sm bg-gray-100 px-2 py-1 rounded">{notification.channel}</code>
              </div>
            )}
            <div>
              ...
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                ...
                notification.priority === 'max' ? 'bg-red-200 text-red-900' :
                'bg-gray-100 text-gray-800'
              }`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-gray-900 text-sm">{formatDateTime(notification.createdAt)}</p>
            </div>
            {notification.sentAt && (
              <div>
                <p className="text-sm font-medium text-gray-500">Sent At</p>
                <p className="text-gray-900 text-sm">{formatDateTime(notification.sentAt)}</p>
              </div>
            )}
          </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs ${
                          notif.status === 'sent' ? 'bg-blue-50 text-blue-700' :
                          notif.status === 'delivered' ? 'bg-green-50 text-green-700' :
                          notif.status === 'opened' ? 'bg-emerald-50 text-emerald-700' :
                          notif.status === 'suppressed' ? 'bg-yellow-50 text-yellow-700' :
                          notif.status === 'dismissed' ? 'bg-gray-50 text-gray-600' :
